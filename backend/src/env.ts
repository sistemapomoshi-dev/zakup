...
const envSchema = z.object({
  ...
  SPACES_SIGNING_REGION: optionalStringSchema,
  ...
  RETAIL_TARGET_MARGIN_PERCENT: z.coerce.number().positive().max(99).default(30),
  RETAIL_SYNC_ENABLED: z
    .enum(['true', 'false'])
    .default('true')
    .transform((value) => value === 'true'),
  YANDEX_GPT_API_KEY: optionalStringSchema,
  YANDEX_GPT_FOLDER_ID: optionalStringSchema,
  YANDEX_GPT_MODEL: stringWithDefault('yandexgpt/latest'),
  YANDEX_GPT_COMPLETION_URL: stringWithDefault(
...
function validateMoyskladEnv(env: z.infer<typeof envSchema>, ctx: z.RefinementCtx) {
  const moyskladConfigured = env.MOYSKLAD_LOGIN !== undefined || env.MOYSKLAD_PASSWORD !== undefined
  if (!moyskladConfigured) return

  if (!env.MOYSKLAD_LOGIN) {
    ctx.addIssue({
      code: 'custom',
      path: ['MOYSKLAD_LOGIN'],
      message: 'MOYSKLAD_LOGIN is required when MoySklad integration is configured',
    })
  }

  if (!env.MOYSKLAD_PASSWORD) {
    ctx.addIssue({
      code: 'custom',
      path: ['MOYSKLAD_PASSWORD'],
      message: 'MOYSKLAD_PASSWORD is required when MoySklad integration is configured',
    })
  }
}
  ...
  for (const key of requiredStorageKeys) {
    if (env[key] === undefined) {
      ctx.addIssue({
        code: 'custom',
        path: [key],
        message: `${key} is required when S3-compatible object storage is configured`,
      })
    }
  }
}
