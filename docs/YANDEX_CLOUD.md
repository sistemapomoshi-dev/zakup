...
On Windows PowerShell, install the Yandex Cloud CLI with:
...
For private attachments and other S3-compatible object storage, add:

```bash
SPACES_REGION=ru-central1
SPACES_SIGNING_REGION=ru-central1
SPACES_BUCKET=<private-bucket-name>
SPACES_ENDPOINT=https://storage.yandexcloud.net
SPACES_ACCESS_KEY_ID=<static-access-key-id>
SPACES_SECRET_ACCESS_KEY=<static-secret-key>
...
The backend storage service is S3-compatible. The env names keep the historical `SPACES_*` prefix, but Yandex Object Storage is configured by setting `SPACES_ENDPOINT=https://storage.yandexcloud.net` and both `SPACES_REGION` and `SPACES_SIGNING_REGION` to `ru-central1`. For private files, keep the bucket private and serve downloads through backend-generated presigned URLs after permission checks. Set `SPACES_CDN_BASE_URL` only for public immutable media delivered through Cloud CDN.
